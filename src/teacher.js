...
function renderRoster(students, isCloud) {
  ...
  // Stats
  const totalStars = students.reduce((s, st) => s + (Number(st.total_stars ?? st.totalStars ?? 0) || 0), 0);
  const totalWords = students.reduce((s, st) => s + (Number(st.total_words ?? st.totalWords ?? 0) || 0), 0);
