...
function updateProgressBar() {
  ...
  const pct = lesson.wordsPerLevel > 0 
    ...
    : 0;
  const roundedPct = Math.round(Math.min(pct, 100));
  fillEl.style.width = roundedPct + '%';

  if (wrapEl) {
    wrapEl.setAttribute('aria-valuenow', roundedPct);
    wrapEl.setAttribute('aria-valuetext', `${roundedPct}% of level complete`);
  }
}
