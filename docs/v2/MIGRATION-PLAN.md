...
| v2 development | Long-lived `nature-quest-v2`; feature branches target it; intentional `/magic-type-quest/v2/` preview | No v2 learner writes yet; reserved `naturequest:v2:` namespace | Use `npm run build:v2` for a preview artifact, never deploy it as production default |
...
3. **Validate and map (#162):** validate the legacy shapes, copy useful evidence to an explicit versioned v2 schema, and record only a non-personal migration version in `naturequest:v2:migration-version`. Never overwrite or delete the original v1 keys. Make import idempotent and test repeat/reload/failure paths before first learner write.
...
No automatic import or final v2 persistence schema is implemented by #156. No personal data is written to migration logs in this stage.
